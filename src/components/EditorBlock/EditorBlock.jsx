    ...
    <div className={`editor-block ${deleting && "editor-child--shrink"}`}>
      <pre className="editor-block__code">
        <div className="editor-block__buttons">
          <button
            className="editor-block__action"
            onClick={() => blockToggle("edit")}
          >
          ...
          <button className="editor-block__action" onClick={addDeleteAnimation}>
            <img
              className="editor-block__icon"
              src={deleteIcon}
              alt="delete row"
            />
          </button>
          <button
            className="editor-row__action"
            onClick={(e) => rowToggle("add")}
          >
            <img className="editor-row__icon" src={addIcon} alt="add row" />
          </button>
        </div>
        <code>
          {`<${block.type} className="${block.name}${formatMods(
            block.modifiers
          )}">`}
          {rows.map((row) => {
            return (
